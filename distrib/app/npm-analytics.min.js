...
(function (moduleName, root, factory) {
  /* eslint-disable */
  if (typeof define === "function" && define.amd) {} else if ((typeof exports === "undefined" ? "undefined" : _typeof(exports)) === "object") {
...
(function () {
  ...
  function updateGraphics() {
    ...
    if (typeSearch === "User") {
      http.getJSON(urlCorsBypass + "https://api.npms.io/v2/search?q=maintainer:" + search).then(function (data) {
        var packages = data.results.map(function (x) {
          return x.package;
        });
        var promises = packages.map(function (x) {
          return http.getJSON(urlCorsBypass + ["https://api.npmjs.org/downloads/range/", startDate.format("YYYY-MM-DD"), ":", endDate.format("YYYY-MM-DD"), "/", x.name].join(""));
        });
        stats = [];
        chartData = {};
        Promise.all(promises).then(function (response) {
          stats = response.filter(function (n) {
            return n !== null;
          });
          redraw();
        });
      ...
      var urlNpm = ["https://api.npmjs.org/downloads/range/", startDate.format("YYYY-MM-DD"), ":", endDate.format("YYYY-MM-DD"), "/", search].join("");
      http.getJSON(urlCorsBypass + urlNpm).then(function (data) {
        stats = [data];
        redraw();
      });
    }
    ...
    if (stats.length > 0) {
      for (var i = 0; i < stats.length; i++) {
        var dataset = {
          ...
          downloads: stats[i].downloads.reduce(function (accu, cur) {
            return accu += cur.downloads;
          }, 0)
        };
  ...
  function redraw() {
    if (stats.length) {
      ...
      redrawArray();
      chartData.labels = stats[0].downloads.map(function (x) {
        return x.day;
      });
      chartData.datasets = [];
      for (var i = 0; i < stats.length; i++) {
        var dataset = {
          label: stats[i].package,
          data: stats[i].downloads.map(function (x) {
            return x.downloads;
          }),
